...
fun Calculator(
    ...
    val maxLength = 30 // maximum length of the text
    val maxFontSize = 80.sp // maximum font size
    val minFontSize = 25.sp // minimum font size

    val textLength = state.number1.length + (state.operation?.symbol?.length ?: 0) + state.number2.length

    val fontSize = when {
        textLength > maxLength -> minFontSize
        else -> maxFontSize * (1 - (textLength.toFloat() / maxLength.toFloat()))
    }
